...
module PSRAM_READER (
    ...
    wire [7:0]  FINAL_COUNT = QPI ? (13 + size*2) : (19 + size*2); // was 21/27: Always read 1 word
    ...
    always @ (posedge clk)
        if((QPI ? (counter >= 14) : (counter >= 20)) && counter <= FINAL_COUNT)
            if(sck)
...
module PSRAM_QPI (
    ...
    always @*
        case (state)
            IDLE: if(q) nstate = QPI; else nstate = IDLE;
