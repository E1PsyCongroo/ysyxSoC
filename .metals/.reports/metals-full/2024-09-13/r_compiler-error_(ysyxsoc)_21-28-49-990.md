...
class APBSPI(address: Seq[AddressSet])(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val noResp      = WireDefault(Flipped(new APBBundle(APBBundleParameters(addrBits = 32, dataBits = 32))))
    noR@@low control signals from the master
    noRpsel      = Output(Bool())
    noRpenable   = Output(Bool())

    noRayload signals
    noRpwrite    = Output(Bool())
    noRpaddr     = Output(UInt(params.addrBits.W))
    noRpprot     = Output(UInt(params.protBits.W))
    noRpwdata    = Output(UInt(params.dataBits.W))
    noRpstrb     = Output(UInt((params.dataBits/8).W))
    noRpauser    = BundleMap(params.requestFields)

    noRpready    = Input(Bool())
    noRpslverr   = Input(Bool())
    noRprdata    = Input(UInt(params.dataBits.W))
    noRpduser    = BundleMap(params.responseFields)

    val noTrans     = WireDefault(new APBBundle(APBBundleParameters(addrBits = 32, dataBits = 32)))
    ...
    val isFlashAccess = address.contains(mspi.io.in.paddr)
