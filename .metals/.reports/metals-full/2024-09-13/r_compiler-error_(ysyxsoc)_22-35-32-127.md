...
class APBSPI(address: Seq[AddressSet])(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    SPIReq.psel     := Mux(isIdle, false.B, true.B)
    SPIReq.penable  := Mux(isIdle, flase.B, true.B)
    SPIReq.pwrite   := Mux()
    SPIReq.paddr    := Output(UInt(params.addrBits.W))
    SPIReq.pprot    := Output(UInt(params.protBits.W))
    SPIReq.pwdata   := Output(UInt(params.dataBits.W))
    SPIReq.pstrb    := Output(UInt((params.dataBits/8).W))
    SPIReq.pauser   := BundleMap(params.requestFields)

    SPIReq.pready   := Mux(isI@@)
    SPIReq.pslverr  := Input(Bool())
